...
GAMES_TOPIC_ID - ID топика для опросов по играм (1282 для продакшена)
```
...
### **Настройка топика:**
Для продакшена измените `GAMES_TOPIC_ID` на `1282` в secrets.

**Важно:** Используется отдельная переменная `GAMES_TOPIC_ID` для избежания конфликтов с опросами по тренировкам, которые используют `ANNOUNCEMENTS_TOPIC_ID`.
