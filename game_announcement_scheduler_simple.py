#!/usr/bin/env python3
"""
Простая система планирования анонсов игр (без топика)
"""
...
class GameAnnouncementSchedulerSimple:
    """Простой планировщик анонсов игр"""
...
⏰ Задача на: {game_info['date']} в 09:00 МСК"""
    ...
    async def process_new_games(self):
        ...
        for game_key, poll_info in self.game_polls.items():
            ...
            if self.should_create_announcement_task(game_info):
                ...
                # Создаем задачу синхронно
                if await self.create_announcement_task(game_info):
                    created_tasks += 1
...
announcement_scheduler = GameAnnouncementSchedulerSimple()

async def main():
    """Основная функция"""
    print("🏀 ПРОСТОЙ ПЛАНИРОВЩИК АНОНСОВ ИГР")
    print("=" * 60)
    ...
    print("\n🔄 Обработка новых игр...")
    await announcement_scheduler.process_new_games()
