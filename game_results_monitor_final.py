...
class GameResultsMonitorFinal:
    ...
    def should_check_results(self) -> bool:
        ...
        print(f"   Время: {now.strftime('%H:%M:%S')} MSK")
        print(f"   День недели: {weekday} ({['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'][weekday]})")
        print(f"   Час: {current_hour}, Минуты: {current_minute}")
        print(f"   Минуты % 15: {current_minute % 15}")
        
        # Проверяем, что минуты кратны 15 (0, 15, 30, 45) или это принудительный запуск
        # Для автоматических запусков по расписанию GitHub Actions минуты всегда кратны 15
        # Но для ручных запусков и отладки разрешаем любые минуты
        if current_minute % 15 != 0:
            # Если это не кратно 15, но время в допустимом диапазоне, все равно разрешаем
            pass
        
        # Будни (Понедельник-Пятница, 0-4)
        if weekday <= 4:  # Понедельник-Пятница
            print(f"   📅 Будни - проверяем время 19:30-00:30")
            # Проверяем время с 19:30 до 00:30 (следующий день)
            if current_hour == 19 and current_minute >= 30:  # 19:30-19:59
                print(f"   ✅ Время 19:30-19:59: {current_hour}:{current_minute}")
                return True
            elif current_hour == 0 and current_minute <= 30:  # 00:00-00:30
                print(f"   ✅ Время 00:00-00:30: {current_hour}:{current_minute}")
                return True
            elif current_hour >= 20:  # 20:00-23:59
                print(f"   ✅ Время 20:00-23:59: {current_hour}:{current_minute}")
                return True
            print(f"   ❌ Время не подходит для будней: {current_hour}:{current_minute}")
            return False
