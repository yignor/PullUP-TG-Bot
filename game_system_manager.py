...
class GameSystemManager:
    ...
    def _is_correct_time_for_announcements(self) -> bool:
        ...
        # Отправляем анонсы в 12:00-12:59 МСК (расширенное окно)
        if now.hour == 12:
            print(f"🕐 Время подходящее для отправки анонсов: {now.strftime('%H:%M')}")
            return True
        
        print(f"⏰ Не время для отправки анонсов: {now.strftime('%H:%M')} (нужно 12:00-12:59)")
        return False
    ...
    async def send_game_announcement(self, game_info: Dict, game_position: int = 1) -> bool:
        ...
        try:
            ...
            # Запускаем мониторинг результатов игры
            if game_link:
                try:
                    from game_results_monitor import start_game_monitoring
                    await start_game_monitoring(game_info, game_link)
                    print(f"🎮 Запущен мониторинг результатов для игры")
                except Exception as e:
