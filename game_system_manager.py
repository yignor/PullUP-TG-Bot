...
class GameSystemManager:
    ...
    def should_create_poll(self, game_info: Dict) -> bool:
        ...
        # Проверяем время выполнения (расширенное окно) только для логирования
        now = get_moscow_time()
        if now.hour == 10:
            print(f"🕐 Время подходящее для создания опросов: {now.strftime('%H:%M')}")
        else:
            print(f"⏰ Не время для регулярного создания опросов: {now.strftime('%H:%M')} (нужно 10:00-10:59)")
            print(f"🆕 Но создаем опрос для новой игры: {game_info['date']}")
