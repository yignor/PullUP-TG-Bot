...
class GameSystemManager:
    ...
    def _is_correct_time_for_polls(self) -> bool:
        ...
        # Создаем опросы в 10:00-13:00 МСК (расширенное окно)
        if 10 <= now.hour <= 12:
            print(f"🕐 Время подходящее для создания опросов: {now.strftime('%H:%M')}")
            return True
        
        print(f"⏰ Не время для создания опросов: {now.strftime('%H:%M')} (нужно 10:00-13:00)")
        return False
    ...
    def _is_correct_time_for_announcements(self) -> bool:
        ...
        # Отправляем анонсы в 10:00-13:00 МСК (расширенное окно для надежности)
        if 10 <= now.hour <= 12:
            print(f"🕐 Время подходящее для отправки анонсов: {now.strftime('%H:%M')}")
            return True
        
        print(f"⏰ Не время для отправки анонсов: {now.strftime('%H:%M')} (нужно 10:00-13:00)")
        return False
