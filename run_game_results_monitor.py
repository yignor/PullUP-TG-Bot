...
async def run_game_results_monitor():
    ...
    hour = now.hour
    if 0 <= hour < 18:
        print(f"📅 Режим: утро/день (запуск каждые 30 минут)")
    else:
        print(f"📅 Режим: вечер (запуск каждые 15 минут)")
    ...
    if not games_to_monitor:
        ...
        print("   - Утром/днем: каждые 30 минут")
        print("   - Вечером (18:00-23:59): каждые 15 минут")
        return
