...
def get_training_locations(week_start_date):
    ...
    # Логика: до определенной недели - СШОР ВО, после - чередование
    # Можно настроить дату перехода на новую логику
    
    # Для примера: неделя 35 (конец августа) - последняя неделя старой логики
    if week_number <= 35:  # Старая логика (до конца августа)
        return {
            'tuesday': {'time': '19:00', 'location': 'СШОР ВО'},
            'friday': {'time': '20:30', 'location': 'СШОР ВО'}
        }
    else:  # Новая логика (с сентября)
